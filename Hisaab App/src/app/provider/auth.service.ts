...
export class AuthService {
  ...
  logIn(loginForm) {
    ...
    return this.http.post('http://localhost:8000/api/authenticate', loginForm, options)
      .map(res => {
 ...
 getUsers(){
    let headers = new Headers({ 'Content-Type': 'application/json' });
    let options = new RequestOptions({ headers: headers });
    return this.http.get('http://localhost:8000/api/get_users', options)
      .map(res => {
        console.log(res.json())
        return res.json()
      })
  ...
  registrationSchool(RegistrationForm) {
    ...
    let headers = new Headers({ 'Content-Type': 'application/json' });
    let options = new RequestOptions({ headers: headers });
    return this.http.post('http://localhost:8000/api/register', RegistrationForm, options)
      .map(res => {
        console.log(res.json())
        return res.json()
      })
